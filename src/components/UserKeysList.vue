  ...
  <div>
    <mdb-modal size="lg" :show="modal" @close="modal = false" info>
      <mdb-modal-header :color="$globalThemeColorDark">
        <mdb-modal-title>UserKey details</mdb-modal-title>
      </mdb-modal-header>
      <mdb-modal-body>
        <div class="d-flex flex-row pb-3">
          <div class="flex-fill">
            <h3>
              <mdb-badge :color="$globalBadgeColor" class="w-100 p-2">ID: {{key.id}}</mdb-badge>
            </h3>
          </div>
        </div>
        <div>
          <mdb-tbl responsive>
            <mdb-tbl-body>
              ...
              <tr>
                ...
                <td>
                  <pre style="overflow:auto;text-overflow:ellipsis;white-space:pre-wrap;width:100%">{{ key.key }}</pre>
                </td>
              </tr>
              <tr>
                <th :class="$globalThemeColor + ' text-center'" style="width: 100px">User</th>
                <td>
                  <a href="#">
    ...
    viewDetails(row) {
      ...
      this.$http
        ...
        .then(response => {
            ...
            this.key.created_at = new Date(this.key.created_at).toLocaleString()
            this.key.updated_at = new Date(this.key.updated_at).toLocaleString()
            this.key.key = this.chunkStr(this.key.key).join("\r\n");
